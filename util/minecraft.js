...
async function pingModernMinecraftServer(server,port,opts) {
    return new Promise(function(a,r) {
        ...
        const conn = net.createConnection(port,server,async function() {
            ...
            var versionCode = writeVarInt(opts.oneSevenProtocolVersion || 754)
            var serverAddress = Buffer.from(server,"utf8")
            var serverAddressLength = writeVarInt(serverAddress.byteLength)
            // Write packet header
...
async function pingMinecraftServer(server,port,opts) {
    return new Promise(function(a,r) {
        ...
        const conn = net.createConnection(port,server,async function() {
            ...
            var hostname = Buffer.from(server,"utf16le").swap16()
...
async function pingAncientMinecraftServer(server,port) {
    return new Promise(function(a,r) {
        ...
        conn.on("data",function(d) {
            ...
            if (data.length != 3) {
                return r("Failed to parse pre-1.4 data")
            }
            console.log(data)
            a({
                version: {
