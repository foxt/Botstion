...
async function pingModernMinecraftServer(server, port, opts) {
    return new Promise((a, r) => {
        ...
        const conn = net.createConnection(port, server, async () => {
            ...
            conn.write(
                new Uint8Array([
                    ...writeVarInt(4 + versionCode.length + serverAddress.length + serverAddressLength.length),
                    0x00
                ])
            );
            // write handshake
...
async function pingMinecraftServer(server, port, opts) {
    return new Promise((a, r) => {
        ...
        const conn = net.createConnection(port, server, async () => {
            ...
            conn.write(
                new Uint8Array([0x00, hostname.byteLength + 7, opts.oneSevenProtocolVersion || 74, 0x00, server.length])
            );
            conn.write(hostname);
...
module.exports = {
    ...
    addons: {
        mcLib: { pingAncientMinecraftServer, pingMinecraftServer, pingModernMinecraftServer }
    }
};
