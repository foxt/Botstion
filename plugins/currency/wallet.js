            ...
            if (c.db.dbLoaded && c.db.tables.wallet) {
                try {
                    ...
                    if (amount < 0) {
                        ...
                        let reply = await m.reply(new Discord.MessageEmbed()
                            .setTitle("Completing transaction...")
                            .setDescription("<:loader:522177421370195969> Give us a mo")
                            .setColor("#3273dc"));
                        c.db.tables.wallet.sync();
                        let payerBal = await c.getcoins(m.author.id);
                        let payeeBal = await c.getcoins(fetched.id);
                        if (payerBal >= amount) {
    ...
    },
    {
        ...
        execute: async(c, m, a) => {
            if (c.db.dbLoaded && c.db.tables.wallet) {
                try {
                    c.db.tables.wallet.sync();
                    let coins = await c.getcoins(m.author);
                    return m.reply(new Discord.MessageEmbed()
