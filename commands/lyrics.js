...
exports.run = async(client,msg,args) => {
    snek.get('https://api.genius.com/search?q=' + url.parse(args.join(" ")).href).set("Authorization", "Bearer " + config.geniusAccessToken).then(function(r) {
        ...
        if (songs.length < 1) {
            ...
            ly.song(songs[0].result.id, {fetchLyrics:true}).then(function (s) {
                ...
                msg.reply({embed:new Discord.RichEmbed().setTitle(s.full_title)
